...
# Vérification de l'accès aux clés d'API
puts "API Key: #{ENV['TWITTER_API_KEY']}"
puts "API Secret: #{ENV['TWITTER_API_SECRET']}"
puts "Access Token: #{ENV['TWITTER_ACCESS_TOKEN']}"
puts "Access Token Secret: #{ENV['TWITTER_ACCESS_TOKEN_SECRET']}"

class TwitterBot
  def initialize
    # Ajoutons un point d'arrêt ici pour vérifier l'initialisation du client
    binding.pry
...
# Méthode pour tester la connexion à l'API Twitter
  def test_connection
    begin
      user = @client.user
      puts "Authentification réussie. Bienvenue, #{user.screen_name}!"
...
# Méthode pour envoyer un message direct à un utilisateur par son nom d'utilisateur
  def send_direct_message(username, message)
    binding.pry
    @client.create_direct_message(username, message)
    puts "Message direct envoyé à @#{username} : #{message}"
  end

# Méthode pour envoyer "Bonjour" à 5 journalistes choisis aléatoirement
  def greet_journalists(journalists)
    ...
    # Envoyer un message à chaque journaliste sélectionné
    selected_journalists.each do |journalist|
      send_direct_message(journalist, "Bonjour ! J'espère que tu passes une bonne journée ! 🌟")
    end
  end

  def promote_hashtag_to_journalists(journalists, message, hashtag)
    # Point d'arrêt pour vérifier les paramètres `journalists`, `message` et `hashtag`
    binding.pry

    journalists.each do |journalist|
      tweet_to_user(journalist, message, hashtag)
    end
  end
end
