...
class XBot
  def initialize
    # Initialisation du client X avec les clés d'API
    @client = X::Client.new do |config|
      config.bearer_token = ENV['X_BEARER_TOKEN']
      config.consumer_key = ENV['X_API_KEY']
      config.consumer_secret = ENV['X_API_SECRET']
  ...
  def send_tweet(username, message)
    ...
    @client.post("tweets", { text: "@#{username} #{tweet}" })
  ...
  def like_hello_world_tweets
    ...
    search_results = @client.get("tweets/search/recent", {
      query: "#hello_world",
      max_results: 25
    })

    # Extraire les IDs des tweets
    tweet_ids = search_results.data.map { |tweet| tweet.id }

    # Liker chaque tweet trouvé
    tweet_ids.each do |tweet_id|
      begin
        # Liker le tweet
        @client.post("users/#{@client.user.id}/likes", { tweet_id: tweet_id })
        puts "Tweet liké : ID #{tweet_id}"
      rescue X::Error => e
  ...
  def follow_hello_world_users
    ...
    user_ids = search_results.data.map { |tweet| tweet.author_id }.uniq

    # Suivre chaque utilisateur qui a tweeté
    user_ids.each do |user_id|
      begin
        # Suivre l'utilisateur
        @client.post("users/#{@client.user.id}/following", { target_user_id: user_id })
        puts "Followé utilisateur ID: #{user_id}"
      rescue X::Error => e
